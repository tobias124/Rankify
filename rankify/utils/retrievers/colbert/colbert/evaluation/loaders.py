...
from rankify.utils.retrievers.colbert.colbert.parameters import DEVICE
from rankify.utils.retrievers.colbert.colbert.modeling.colbert import ColBERT
from rankify.utils.retrievers.colbert.colbert.utils.utils import print_message, load_checkpoint
from rankify.utils.retrievers.colbert.colbert.evaluation.load_model import load_model
from rankify.utils.retrievers.colbert.colbert.utils.runs import Run
...
def load_collection(collection_path):
    ...
    # Get total lines in file for tqdm progress bar
    with open(collection_path) as f:
        total_lines = sum(1 for _ in f)

    with open(collection_path) as f:
        for line_idx, line in tqdm(enumerate(f), total=total_lines, unit="lines", desc="Loading Collection"):
